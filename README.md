...
1. Beneath this header, provide a 100-word summary of the visualization — what specifically does the data viz show or communicate? [3 points]
1. Include a quotation from *Data Feminism* (or another source) about the data visualization or its significance somewhere in this summary. Format this quotation as a block quote. (This quotation does not count for your 100-word summary). [3 points]
1. Beneath the summary, include an image of the data visualization by either linking to it with an image URL from the *Data Feminism* textbook or by downloading the image and including it as a file in this repository. Some images from *Data Feminism* will not work as image URLs and must be downloaded and included as a file. Your image should display properly on GitHub, so if it's not showing up, you haven't used the correct file path. [5 points]
...
1. In this section, you will answer the following questions. Bold these questions, make them an ordered list, and then write answers (not bolded) beneath the questions:
  ...
  - Who is the audience for the visualization? Who might be impacted, either directly or indirectly, by the visualization?
  - What values, goals, or interests are at stake with the visualization?
  - What can be learned from the visualization?
  - What issues, if any, related to power and justice are reflected in the data visualization?
